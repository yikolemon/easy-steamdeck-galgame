                ...
                if data[offset] == 0x00:  # Section start
                    ...
                    while offset < len(data) - 1:
                        ...
                        if type_byte == 0x08:  # End of section
                            ...
                            if value_end == -1:
                                break
                            value = data[offset:value_end].decode('utf-8', errors='ignore')
                            offset = value_end + 1
    ...
    def add_non_steam_game(
        ...
        try:
            ...
            for shortcut in shortcuts:
                if shortcut.get('appname') == app_name or shortcut.get('exe') == f'"{exe_path}"':
                    return False, f"ERROR: Game '{app_name}' already exists in Steam library"
...
def get_game_search_paths() -> List[str]:
    """Get configured game search paths from config"""
    config = Config.load_config()
    return config.get('game_search_paths', [])


def set_game_search_paths(paths: List[str]):
    """Save game search paths to config"""
    Config.save_config({'game_search_paths': paths})


def add_game_search_path(path: str) -> Tuple[bool, str]:
    """Add a new game search path"""
    if not os.path.exists(path) or not os.path.isdir(path):
        return False, "ERROR: Path does not exist or is not a directory"
    
    paths = get_game_search_paths()
    if path in paths:
        return False, "ERROR: Path already in list"
    
    paths.append(path)
    set_game_search_paths(paths)
    return True, "SUCCESS: Path added"


def remove_game_search_path(path: str) -> Tuple[bool, str]:
    """Remove a game search path"""
    paths = get_game_search_paths()
    if path not in paths:
        return False, "ERROR: Path not in list"
    
    paths.remove(path)
    set_game_search_paths(paths)
    return True, "SUCCESS: Path removed"
